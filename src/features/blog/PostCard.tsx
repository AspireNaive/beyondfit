...
/** The newest post, laid out wide at the top of the feed. */
export function FeaturedPost({ post, basePath = '/blog' }: { post: Post; basePath?: string }) {
  const href = `${basePath}/${post.slug}`
  return (
    <article className="group grid overflow-hidden rounded-2xl border border-ink-700 bg-ink-850/70 transition-colors hover:border-ink-500 lg:grid-cols-[1.25fr_1fr]">
      <Link to={href} className="block" aria-hidden tabIndex={-1}>
        <PostCover post={post} className="aspect-[16/10] h-full lg:aspect-auto lg:min-h-[22rem]" />
      </Link>
      <div className="flex flex-col p-6 sm:p-8">
        ...
        <h2 className="mt-4 text-4xl leading-none text-balance sm:text-5xl">
          <Link to={href} className="transition-colors hover:text-volt-400">
            {post.title}
          </Link>
        </h2>
        <p className="mt-4 flex-1 text-base leading-relaxed text-chalk-dim text-pretty">{post.excerpt}</p>
        <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-ink-700 pt-5">
          ...
          <p className="flex items-center gap-2 text-xs text-chalk-faint">
