...
import type { CartLine, Order, PaymentMethod } from '@/domain/commerce/model'
...
export function usePlaceOrder(customer: UserProfile | null) {
  ...
  return useMutation({
    mutationFn: ({ lines, method }: { lines: readonly CartLine[]; method: PaymentMethod }) =>
      container.orders.placeOrder(lines, customer!, { method }),
    onSuccess: () => {
